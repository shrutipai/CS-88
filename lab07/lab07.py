...
class Keyboard:
    ...
    def press(self, name):
        ...
        if name in self.buttons.keys():
            b_name = self.buttons[name]
            b_name.pressed += 1
            return b_name.letter
        return ''
...
class T88ble():
    ...
    def column(self, label):
        ...
        """
        "*** YOUR CODE HERE ***"
        our_index = self.column_labels.index(label)
        values = []
        for each in self.rows:
            values.append(each[our_index])
        return values 
    ...
    def select(self, labels):
        ...
        for i in range(0, len(labels)):
            indices.append(self.column_labels.index(labels[i]))

        result_rows = []
        for i in new_rows:
            inside_row = []
            for a in indices:
                inside_row.append(i[a])
            result_rows.append(inside_row)

        new_table = T88ble(result_rows, new_labels)
        return new_table
    ...
    def sort(self, label, descending=True):
        ...
        for i in self.rows:
            column_values.append(i[index])

        sorted_values = sorted(column_values, reverse = descending)
    ...
    def where(self, label, filter_fn):
        ...
        new_table = T88ble(result_rows, self.column_labels)
